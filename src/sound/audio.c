...
/* The rate at which the volume fades when musics transition. The higher number indicates music
 * fading faster */
...
Audio* createAudio(const char* filename, uint8_t loop, int volume) {
    ...
    if (SDL_LoadWAV(filename, &(newAudio->audio), &(newAudio->bufferTrue),
                    &(newAudio->lengthTrue)) == NULL) {
        fprintf(stderr, "[%s: %d]Warning: failed to open wave file: %s error: %s\n", __FILE__,
                __LINE__, filename, SDL_GetError());
        free(newAudio);
        return NULL;
    }
